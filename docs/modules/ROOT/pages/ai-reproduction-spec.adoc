...
On any platform that supports blur (CSS `backdrop-filter`, native Material/blur APIs, `BackdropFilter`, etc.), simultaneously render a *single block-wide glass layer* over the code panel (not per-token). *Probe at runtime* — use cross-fade only when blur is unavailable. See xref:glass-lens-capabilities.adoc[glass-lens-capabilities.md].
